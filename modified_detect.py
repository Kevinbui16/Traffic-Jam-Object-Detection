...
frame_time = 0.060
speed_threshold = 10
traffic_threshold = 30 #example traffic threshold for number of cars
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if c not in current_positions:
                        current_positions[c] = []
                    current_positions[c].append((x_center.item(), y_center.item()))
            ...
            for label, positions in current_positions.items():
                for idx, current_pos in enumerate(positions):
                    ...
                    if object_id in previous_positions:
                        ...
                        if speed_km_per_h < 300:  # Filter out unrealistic speeds
                            print(f"Object ID: {object_id}, Speed: {speed_km_per_h:.2f} km/h")
                            print(len(positions))

                            if (label == 0 or label == 1 or label == 2 ) and speed_km_per_h < speed_threshold and len(positions) >= traffic_threshold:
                                traffic_jam_detected = True
                                print("Traffic Jam detected!")

            previous_positions.update({f"{label}_{idx}": pos for label, pos_list in current_positions.items() for idx, pos in enumerate(pos_list)})

            if traffic_jam_detected:
                print("traffic congestion")

                cv2.putText(im0, 'Traffic Jam', (150, 150), cv2.FONT_HERSHEY_SIMPLEX, 2.5, (0, 255, 0), 5, cv2.LINE_AA)
            else:
                print("traffic clear")
                cv2.putText(im0, 'Clear Traffic', (150, 150), cv2.FONT_HERSHEY_SIMPLEX, 2.5, (0, 255, 0), 5, cv2.LINE_AA)
